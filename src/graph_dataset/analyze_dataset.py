    ...
    return pl.DataFrame(
        {
            ...
            "number_of_nodes": [
                len(db_instance[index_].x) for index_ in range(len(db_instance))
            ],
            "number_of_edges": [
                len(db_instance[index_].edge_attr)
                for index_ in range(len(db_instance))
            ],
            "no_loops_or_island": [
                len(db_instance[index_].edge_attr) + 1
                == len(db_instance[index_].x)
                for index_ in range(len(db_instance))
            ],
        }
    )
